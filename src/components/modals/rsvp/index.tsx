...
import { useCal } from "@/components/calendar/calendar-provider";
...
export function EventDialog({
  ...
  const { refetchRsvps, calendar } = useCal();
  const { data: rsvpsForEvent, refetch: refetchRsvpsForEvent } =
    api.rsvp.getAllRsvpForAnEvent.useQuery(
      { eventId: event.uid },
      { enabled: !!event.uid && isOpen },
    );
  ...
  const { mutate: createRsvp } = api.rsvp.createRsvp.useMutation({
    onSuccess: async () => {
      ...
      void sendRsvpEmail({
        event: { name: event.summary, start: event.start, end: event.end },
        user: {
    ...
    <Modal isOpen={isOpen} onOpenChange={onClose} key={event.uid}>
      <ModalContent>
        {loading ? (
          ...
          <RSVPChange status={success} />
        ) : (
          <RSVPContent
            isRsvped={isRsvped ?? false}
            event={event}
            metadata={metadata}
            rsvpsForEvent={rsvpsForEvent}
            plusOne={plusOne}
            maxPlusOne={maxPlusOne}
  ...
  function RSVPChange({ status }: { status: RSVPStatus }) {
    return (
      <div className="flex min-h-60 flex-col">
        <ModalHeader className="flex flex-col">
          ...
          <p className="text-sm font-normal text-gray-500">
            {status === "rsvped"
              ...
              : "You have successfully canceled your RSVP for this event."}{" "}
            The host will be notified of your change.
          </p>
        </ModalHeader>
        ...
        <ModalFooter className="w-full flex-row">
      ...
      <>
        <ModalHeader className="flex flex-col gap-1">
          ...
          <p className="text-sm font-normal text-gray-500">
            {isRsvped
              ? "You have already RSVPed for this event."
              : "Please let me know if you're coming to this event and whether you'll be bringing a plus one"}
          </p>
        </ModalHeader>
        <ModalBody>
            ...
            {!!rsvpsForEvent?.length && rsvpsForEvent.length > 0 && (
              <IconLabel icon={Users} labelClass="">
                {rsvpsForEvent.map((rsvp) => (
                  <Badge
                    key={rsvp.userId}
                    className="gap-1 border border-gray-300 bg-gray-200 pl-1 pr-2 text-gray-600 hover:bg-gray-300"
                  >
                    <Avatar
                      key={rsvp.userId}
                      className="h-[16px] w-[16px] border border-gray-300 bg-gray-200 text-gray-500"
                    >
        ...
        <ModalFooter className="w-full flex-row">
          {isRsvped ? (
            ...
            <div className="flex w-full flex-row justify-between gap-2">
              ...
              <Button className="ml-auto" onClick={handleRSVP}>
                RSVP
              </Button>
            </div>
          )}
